...
from OMNITRIXGUI import Ui_MainWindow
...
from listen import TranslationHinToEng
from main import MainTaskExecution
from qna import QuestionAnswer
from speck import Speak
...
def qrcode():
    ...
    speak("Enter text to convert to QR code ")
    input_text = input("Enter text to convert to QR code: ")
...
class MainThread(QThread):
    ...
    def MicExecution(self):
        data = listen()
        query = TranslationHinToEng(data)
        return query 
    ...
    def TaskExecution(self):
            speak("Omnitirx Access granted")
            wish()
            while True:
                self.query = self.Listen()
                self.query = str(self.query)
                ReturnData = MainTaskExecution(self.query)
                ...
                if len(self.query) < 3:
                    ...
                    pass
                elif "access master control" in self.query:
                    Speak("ok.. now  master control started , code : OMNITRIX ")
                elif "what is" in self.query or "who is" in self.query or "where is" in self.query or "question" in self.query or "answer" in self.query:
                    Reply = QuestionAnswer(self.query)
                    Speak(Reply)
                ...
                #     speak("message has been sent")
                    ...
                    speak("sir which song you prefer to here")
                    cm = MainThread().Listen().lower()  
                    kit.playonyt(f"{cm}")
                ...
                elif "email to someone" in self.query:
                ...
                # # elif "text to QR code" in self.query or "QR code" in query:
                # #      # get input text from user
                # #     speak("Enter text to convert to QR code ")
                # #     input_text = input("Enter text to convert to QR code: ")
                ...
                elif "bye have a nice day" in self.query:
                    return
                
                else:

                    Reply = ReplyBrain(self.query)
                    Speak(Reply)
...
class Main(QMainWindow):
    ...
    def startTask(self):
        self.ui.movie = QtGui.QMovie("C:\\My Web Sites\\007\\jarvis\\gifs\\G.U.I Material\\B.G\\Black_Template.jpg")  
        self.ui.label.setMovie(self.ui.movie)  
        self.ui.movie.start()  
        self.ui.movie = QtGui.QMovie("C:\\My Web Sites\\007\\jarvis\\gifs\\G.U.I Material\\VoiceReg\\Siri.gif")  
        self.ui.label_2.setMovie(self.ui.movie)  
        self.ui.movie.start()  
        self.ui.movie = QtGui.QMovie("C:\\My Web Sites\\007\\jarvis\\gifs\\G.U.I Material\\ExtraGui\\Earth.gif")  
        self.ui.label_3.setMovie(self.ui.movie)  
        self.ui.movie.start()  
        self.ui.movie = QtGui.QMovie("C:\\My Web Sites\\007\\jarvis\\gifs\\G.U.I Material\\ExtraGui\\Code_Template.gif")  
